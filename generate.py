...
# -*- coding: utf-8 -*-
"""
Dagstarter / automatiseren-boekje voor groep 7.
...
def gen_automatiseren(rnd, t):
    ...
    while len(items) < 8 and pog < 500:
        ...
        if t < 0.25:
            ...
            if rnd.random() < 0.5:
                a, b = rnd.randint(11, 20), rnd.randint(2, 9)
            else:
    ...
    while len(items) < 14 and pog < 500:
        ...
        if t < 0.25:
            ...
            deler, quot = rnd.randint(2, 10), rnd.randint(3, 10)
            rest = 0
        elif t < 0.75:
            deler, quot = rnd.randint(3, 10), rnd.randint(4, 12)
...
def gen_hoofdrekenen(rnd, t):
    ...
    while len(items) < 6 and pog < 500:
        ...
        if t < 0.25:
            ...
            a, b = rnd.randint(110, 799), rnd.randint(105, 399)
        elif t < 0.75:
...
def _redactie_moeilijk(rnd):
    ...
    keuze = rnd.randint(0, 3)
    if keuze == 0:
        prijs = rnd.randint(4, 12) * 1000  # hele euro's in centen
        pct = rnd.choice([10, 25, 50])
...
def _wissel_procenten(rnd, t):
    items = []
    for _ in range(4):
        pct = rnd.choice([10, 25, 50, 100])
        geheel = rnd.randint(2, 20) * 100
        deel = geheel * pct // 100
...
def _wissel_breuken(rnd, t):
    """Deel van een geheel: 1/4 van 20, 3/4 van 100. (groep 7)"""
    breuken = [(1, 2), (1, 4), (3, 4), (1, 5), (2, 5), (1, 10), (3, 10)]
    items = []
    for _ in range(4):
        teller, noemer = rnd.choice(breuken)
        geheel = rnd.randint(2, 12) * noemer  # deelbaar, mooie uitkomst
        antwoord = geheel * teller // noemer
        items.append({"vraag": f"{teller}/{noemer} van {geheel} =",
                      "antwoord": str(antwoord)})
    return "Breuken: deel van een geheel", items
...
def gen_wissel(rnd, t, dagnummer):
    ...
    laat = [_wissel_tijd, _wissel_geld, _wissel_meten, _wissel_meetkunde,
            _wissel_breuken, _wissel_procenten]
...
# ===========================================================================
LEESWOORDEN = [
    # eenvoudig (2 lettergrepen, bekend)
    "vandaag", "morgen", "voetbal", "muziek", "gitaar", "meester", "juffrouw",
    "kanjer", "spelen", "rennen", "lachen", "koning", "tijger", "olifant",
    "banaan", "tomaat", "zomer", "winter", "school", "vakantie",
    # iets langer (3 lettergrepen, klankgroepen)
    "familie", "computer", "kalender", "station", "gordijn", "fontein",
    "muzikant", "kabouter", "chocola", "machine", "paraplu", "avontuur",
    "spannend", "grappig", "gezellig", "vrolijk", "moeilijk", "eerlijk",
    "prachtig", "duidelijk", "opnieuw", "misschien", "eigenlijk", "natuurlijk",
    "belangrijk", "gemakkelijk", "verschillende", "plotseling", "verjaardag",
    # moeilijker (leenwoorden, -tie, samenstellingen)
    "politie", "informatie", "traktatie", "kilometer", "temperatuur",
    "museum", "restaurant", "chauffeur", "journaal", "garage", "bagage",
    "energie", "batterij", "horloge", "fantastisch", "technisch", "tropisch",
    "elektrisch", "publiek", "techniek", "fabriek", "voetbalstadion",
    "boterham", "ziekenhuis", "brandweerman", "verkeerslicht", "huiswerk",
    "tandenborstel", "verrekijker", "gereedschap", "boodschappen",
    "handtekening", "gebeurtenis", "voorstelling", "tentoonstelling",
    "nieuwsgierig", "aantrekkelijk", "geheimzinnig", "onafhankelijk",
    "geschiedenis", "aardrijkskunde", "kampioenschap", "trainingskamp",
    "voorjaarsvakantie", "herfstvakantie", "sinterklaasfeest", "avondvierdaagse",
]
